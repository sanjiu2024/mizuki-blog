...
- 封装 `src/db/search.ts` 的 `searchPosts(db, q)` 用 `prepare + MATCH + bm25/snippet` raw SQL
- 页面 `src/pages/search.astro` 与 API `src/pages/api/search.ts` 复用同一封装
- 无 DB 时降级 `LIKE` 模糊
...
CI 见 `.github/workflows/ci.yml`，需在 GitHub Secrets 配置 `CLOUDFLARE_API_TOKEN` / `CLOUDFLARE_ACCOUNT_ID` / `BETTER_AUTH_SECRET`。
